...
function setup_vendor() {
    ...
    export WAVE_ROOT="$3"
    if [ ! -d "$WAVE_ROOT" ]; then
        echo "\$WAVE_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$WAVE_ROOT/$OUTDIR" ]; then
        mkdir -p "$WAVE_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$WAVE_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$WAVE_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$WAVE_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$WAVE_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        printf " * Copyright (C) $YEAR Styx Project\n" >> $1
    elif [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
        printf " * Copyright (C) 2019-$YEAR Styx Project\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR Styx Project\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 Styx Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR Styx Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR Styx Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR Styx Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR Styx Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$WAVE_ROOT"/prebuilts/tools-wave/common/smali/baksmali.jar
        export SMALIJAR="$WAVE_ROOT"/prebuilts/tools-wave/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$WAVE_ROOT"/prebuilts/tools-wave/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$WAVE_ROOT"/prebuilts/tools-wave/${HOST}-x86/bin/compact_dex_converter
    fi

    if [ -z "$PATCHELF" ]; then
        export PATCHELF="$WAVE_ROOT"/prebuilts/tools-wave/${HOST}-x86/bin/patchelf
    fi
    ...
    if [ ! -f "$WAVE_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$WAVE_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$WAVE_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$WAVE_ROOT"/vendor/wave/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$WAVE_ROOT"/"$OUTDIR"/radio
